                    ...
                    <div class="d-flex justify-content-between">
                        <div class="d-flex align-items-center gap-2">  
                            ...
                            <div class="d-flex flex-column">
                                ...
                                <small class="text-dark">{{ date('F d, Y g:i A', strtotime($document->upload_date)) }}</small>
                            </div>
                        </div>
                                ...
                                @can('delete', $document)
                                    <li>
                                        <form hx-post="{{ route('documents.destroy', ['document' => $document->id]) }}" hx-target="body">
                                            @csrf
                                            @method('delete')
                                            <button class="dropdown-item" type="submit" onclick="return confirm('Are you sure to delete Document?')">
                                                Delete
                                            </button>
                                        </form>
                                    </li>
